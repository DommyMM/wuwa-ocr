...
def main() -> None:
    # Exercise the SHIPPED code path, not a copy of it. identify_echo() includes the hue
    # arbitration that resolves the two near-ties (Fleurdelys vs Leviathan, margin 0.008).
    from wuwa_scanner.identify import _load, identify_echo
    ...
    print(f"{'tile':6s} {'gold':>10s} {'top1':>10s} {'score':>6s} {'marg':>6s} {'via':>8s}  name")
    print("-" * 92)
    ok = 0
    by_hue = 0
    t_all = 0.0
    for i, (gid, note) in enumerate(GOLD):
        ...
        x0 = lat["col_x"][c]
        art = L.crop(img, L.sub_box((x0, y0, x0 + L.TILE_W, y0 + L.TILE_H), L.TILE_ART))

        t0 = time.perf_counter()
        res = identify_echo(art)
        ...
        print(f"r{r}c{c}  {gid:>10s} {str(res['id']):>10s} {res['score']:6.3f} "
              f"{res['margin']:6.3f} {res['via']:>8s}  {tag} {NAME.get(res['id'], '?')}"
              f"{'  [' + note + ']' if note else ''}")
